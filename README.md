...
Начальным мы считаем нетерминал [NtermStart]S[NtermEnd], если его не будет программа не сработает.
...
[TermStr] -> [a-z]
 ...
 ```
Все нетерминалы в грамматике, которые встречаются в ней, но не расписаны, пользователь может задать самостоятельно.
По умолчанию программа задаёт эти параметры, опираясь на встроенный синстаксис. Ниже вы можете ознакомиться с дефолтными значениями. 
